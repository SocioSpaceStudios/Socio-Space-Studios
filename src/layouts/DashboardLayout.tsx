...
const DashboardLayout: React.FC = () => {
  const { user, theme, toggleTheme, toasts, removeToast } = useData();
  ...
  const getPageSubtitle = () => {
    switch (location.pathname) {
      case '/': return "Here's your creator dashboard overview";
      case '/profile': return "Your professional summary and activity log";
      case '/content-planner': return "Plan and organize your content calendar";
      case '/brand-outreach': return "Track your brand partnerships and pitches";
      case '/scripts': return "AI-powered content ideas tailored to your niche";
      case '/ugc-jobs': return "Manage your user-generated content projects";
      case '/finance': return "Advanced financial analytics and management";
      case '/media-kit': return "Create your professional media kit for brand pitches";
      case '/communications': return "Manage team and brand messages";
      case '/team': return "Invite collaborators and manage permissions";
      case '/settings': return "Manage your account preferences";
      case '/templates': return "Store and reuse outreach, caption, and workflow templates";
      case '/ai-assist': return "Use AI helpers to generate hooks, scripts, pitches, and content ideas";
      default: return "";
    }
  };
  ...
  return (
    <div className="flex h-screen bg-background text-textMain overflow-hidden font-sans transition-colors duration-300 relative">
        ...
        {toasts.map((toast) => (
          <div 
            ...
            className={`pointer-events-auto flex items-center gap-3 p-4 rounded-lg shadow-xl border animate-in slide-in-from-right-10 fade-in duration-300 min-w-[280px] md:min-w-[300px]
              ${toast.type === 'success' ? 'bg-surface border-green-500 text-textMain' : 
                toast.type === 'error' ? 'bg-surface border-red-500 text-textMain' : 
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 bg-black/50 z-40 lg:hidden backdrop-blur-sm" onClick={toggleMenu} />
      )}
      ...
      `}>
        ...
        <div className="p-6 flex justify-between items-center lg:block">
          ...
          <button onClick={toggleMenu} className="lg:hidden text-white/80 hover:text-white">
            <X size={24} />
          </button>
        </div>
            ...
            <div key={index}>
              ...
              <div className="space-y-1">
                {group.items.map((item) => (
                  ...
                  >
                    <item.icon className={`w-4 h-4 mr-3 ${location.pathname === item.to ? 'text-primary' : 'text-white'}`} />
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <header className="flex items-center justify-between h-16 md:h-auto md:min-h-[5rem] py-2 md:py-4 px-4 md:px-10 bg-background print:hidden border-b border-borderColor lg:border-none">
          <div className="flex items-center lg:hidden">
            <button onClick={toggleMenu} className="text-textMuted hover:text-textMain mr-3 p-1">
              <Menu size={24} />
            </button>
            <span className="font-bold text-lg text-textMain truncate">Socio Space</span>
          </div>
          
          <div className="hidden lg:block">
            {/* Contextual Header based on Page */}
          ...
          <div className="flex items-center space-x-2 md:space-x-4">
            ...
            <Link to="/profile" className="flex items-center space-x-3 pl-2 md:pl-4 border-l border-borderColor hover:opacity-80 transition-opacity">
              <div className="text-right hidden sm:block">
                <p className="text-sm font-bold text-textMain">{user.name}</p>
                <p className="text-xs text-textMuted">Pro Plan</p>
              </div>
              <img src={user.avatarUrl} alt="User" className="w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-surface shadow-sm object-cover" />
            </Link>
          </div>
        </header>
